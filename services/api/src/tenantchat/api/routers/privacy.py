...
from tenantchat.api.dependencies import Audit, Privacy, Registry, get_settings
...
from tenantchat.api.store import AuditEventRecord
...
async def export_subject(
    ...
    await audit.record(
        AuditEventRecord(
            ...
            resource_id=str(contact.kind.value),
            details={"sessions": len(session_ids)},
        )
    )
...
async def request_deletion(
    ...
    await audit.record(
        AuditEventRecord(
            ...
            resource_id=str(record.request_id),
            details={},
        )
    )
