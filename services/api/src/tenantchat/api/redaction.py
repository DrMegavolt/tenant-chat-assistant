...
class PiiLogFilter(logging.Filter):
    ...
    Installed on the root logger by the composition root; installing twice is
    harmless because redaction is idempotent.
    """
...
def install_pii_log_filter() -> None:
    """Attach the filter to the root logger, once per process."""
    root = logging.getLogger()
    if not any(isinstance(existing, PiiLogFilter) for existing in root.filters):
