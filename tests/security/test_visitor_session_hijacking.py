...
class TestSessionHijacking:
    ...
    def test_a_non_ascii_credential_is_refused_rather_than_crashing(
        ...
        """A header byte above 0x7F must not reach the HMAC comparison.

        `hmac.compare_digest` and `str.encode("ascii")` both raise on non-ASCII
        input, so a single unprintable byte in either half of the token used to
        answer an unauthenticated route with a 500 and a stack trace instead of
        the refusal every other malformed credential gets.
        """
        for token in (b"tc.v1.QQ.\xff", b"tc.v1.\xff.QQ"):
            response = client.get(
                "/api/chat/session",
                headers={VISITOR_CREDENTIAL_HEADER.encode("ascii"): token},
            )
