...
For a local MicroK8s deployment, keep source values in `.local/k8s/`, which is
...
requires authenticated requests, and fails closed.
...
[operator-access runbook](../docs/runbooks/demo-access.md).
...
The flat-YAML `make deployment-security` and `make image-contracts` gates do
not parse the chart's Go templates. `make keycloak-render` renders the chart for
review; every image in the output must end in an exact digest.
